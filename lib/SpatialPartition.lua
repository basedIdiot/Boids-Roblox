...
function SpatialHash.__index:RemoveBoid(Boid)
	if Boid.Previous then
		Boid.Previous.Next = Boid.Next
	end
	if Boid.Next then
		Boid.Next.Previous = Boid.Previous
	end
end
...
function SpatialHash.__index:GetHashesInRange(Boid, Range: number): { number: number }
	...
	local LowerBoundX, UpperBoundX =
		math.clamp(BoidPosition - CellRange, 0, self.Length), math.clamp(BoidPosition + CellRange, 0, self.Length)
	local LowerBoundY, UpperBoundY =
		math.clamp(BoidPosition - CellRange, 0, self.Height), math.clamp(BoidPosition + CellRange, 0, self.Height)
	local LowerBoundZ, UpperBoundZ =
		math.clamp(BoidPosition - CellRange, 0, self.Width), math.clamp(BoidPosition + CellRange, 0, self.Width)
